...
class AllValidatorTest extends Specification {

    AllValidator validator
    Pair<Object, Object[]> test

    def setup() {
        validator = new AllValidator('test')
    }
    ...
    def "value with object which is null in list is invalid"() {
        def empty = null
        Object[] objs = [empty] as Object[]
        test = new Pair("test", objs)

        expect:
        validator.isValid(test)[0] == "validation.choice.all"
    }

    def "value with objects which are null in list is invalid"() {
        def empty = null
        def empty2 = null
    ...
    def "no value with objects which is something in list is invalid"() {
    ...
    def "no value with 1 object which is something in list is invalid"() {
    ...
    def "no value with other object using java 7 notation which is something in list is invalid"() {
        ...
        expect:
        validator.isValid(null, "something", "more", 1, null)[0] == "validation.choice.all"
    }

    def "value with other object using java 7 notation last element is null which is something in list is invalid"() {

        expect:
        validator.isValid("me", "something", null)[0] == "validation.choice.all"
    }
    ...
    def "no value with objects which are null in list but optional is invalid"() {
        def empty = null
        def empty2 = null
    ...
    def "no value with no objects in list but optional is valid"() {
        def empty = null
        def empty2 = null
